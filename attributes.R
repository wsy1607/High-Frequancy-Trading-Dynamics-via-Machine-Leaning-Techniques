...
#Input arguments:
#stock: stock data in LOB
#a: the begining index in stock as the training data set
#k: the length of training data set (number of rows)
#t: number of events defined as a unit second
#p: the proportion of market order against cancellation
#bigT: the time scale in calculation of v8
#smallT: the time scale in calculation of v8
#s: number of steps to be predicted
get_attributes <- function(stock,a,k,t=300,p=0.5,bigT=100,smallT=10,s=5){
  data <- stock[(a+1):(a+k),]
  n <- nrow(stock)
  ...
  y <- as.factor(y)
  #output
  return(data.frame(y,v1,v2,v3,v4,v5,v6,v7,v8,v9))
}
